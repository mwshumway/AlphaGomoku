...
import numpy as np
from utils import prepare_single_input, softmax
import copy
...
class MCTS:
    ...
    def playout(self, env):
        ...
        state, player = env.get_state(), env.current_player
        normalized_state = state * player

        # Evaluate the leaf using the model
        input_data = prepare_single_input(normalized_state, player, action)
        action_probs, value = self.model(input_data)
        ...
        if env.done:
            ...
            node.expand(legal_action_probs, available_actions)
